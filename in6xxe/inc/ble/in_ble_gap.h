...
/**
 ****************************************************************************************
 * @defgroup IN_BLE_GAP BLE General Access Profile  
 * @ingroup BLE
 * @brief  BLE GAP
 * @{
 ****************************************************************************************
...
typedef struct 
{
    /// Authentication information, @see enum in_ble_auth.
    uint8_t info;
...
typedef struct 
{
	/// IO capabilities, @see enum in_ble_io_cap.
    uint8_t iocap;	
	/// OOB present, @see enum in_ble_oob.
    uint8_t oob;		
	/// Authentication, @see enum in_ble_auth.
    uint8_t auth;	
	/// Encryption key size (7 to 16).  
    uint8_t key_size;	
	/// Initiator key distribution, @see enum in_ble_kdist.
    uint8_t ikey_dist;	
	/// Responder key distribution, @see enum in_ble_kdist.
    uint8_t rkey_dist;	
	/// Device security requirements (minimum security level), @see enum in_ble_sec_req.	
    uint8_t sec_req;	
} ble_pairing_t;
...
typedef struct 
{
	/// PHY for transmission, @see enum in_ble_phy_value.
    uint8_t tx_phy;		
	/// PHY for reception, @see enum in_ble_phy_value.
    uint8_t rx_phy;		
} ble_phy_ind_t;
...
/// Test PHY structure
typedef struct {
	/// Test mode operation, @see in_ble_test_mode 
	int test_mode;					
	...
	/// Packet Payload type (only valid for TX mode), @see enum in_ble_pkt_pld_type
	uint8_t tx_pkt_payload;		
	/// Test PHY rate @see enum in_ble_test_phy
	uint8_t phy;						
	/// Modulation Index (only valid for RX mode),  @see enum in_ble_modulation_idx
...
typedef struct 
{
    /// Device Role: Central, Peripheral, Observer, Broadcaster or All roles @see enum in_ble_role 
    uint8_t role;
    ...
    /// Privacy configuration bit field (@see enum in_ble_priv_cfg for bit signification)
    uint8_t privacy_cfg;

    /// -------------- Security Config -----------------------

    /// Pairing mode authorized (@see enum in_ble_pairing_mode)
	...
	/// Device Name write permission @see in_ble_write_att_perm
	uint8_t att_devname_write_perm;		
	/// Device Appearance write permission @see in_ble_write_att_perm
    ...
    /// ------------------ LE PHY Management  -------------------------
    /// Preferred LE PHY for data transmission @see enum in_ble_phy
    uint8_t tx_pref_phy;
    /// Preferred LE PHY for data reception @see enum in_ble_phy
    uint8_t rx_pref_phy;

} ble_set_dev_config_t;
...
struct adv_prim_cfg
{
	...
	/// Advertising channel mapping, @see enum in_ble_adv_chnl 
	uint8_t chnl_map;				
	/// Primary advertising PHY, @see enum in_ble_phy_value
...
struct adv_second_cfg
{
	...
	uint8_t max_skip;	
	/// Secondary advertising PHY, @see enum in_ble_phy_value
	uint8_t phy;			
	/// Advertising SID. 
	uint8_t adv_sid;		
};
...
typedef struct 
{
	/// advertising type, @see enum in_ble_adv_type 
	uint8_t type;											
	/// advertising mode, @see enum  in_ble_adv_disc_mode
	uint8_t disc_mode;								
	/// advertising properties, @see enum in_ble_adv_legacy_prop, @see enum in_ble_adv_ext_prop, @see enum in_ble_adv_per_prop 
	uint16_t prop;
	/// Maximum power level at which the advertising packets have to be transmitted (between -127 and 126 dBm). 										
	int8_t max_tx_pwr;									
	/// Advertising filtering policy, @see enum in_ble_adv_filt_pol
	uint8_t filter_pol;									
...
typedef struct
{
	/// Scan type, @see enum in_ble_scan_type
    uint8_t type;	
	/// Scan properties, @see enum in_ble_scan_prop.
    uint8_t prop;	
	/// Duplicate packet filtering policy, @see enum in_ble_scan_dup_filt.
...
typedef struct 
{
	/// Initiator type, @see enum in_ble_initiator_type.
    uint8_t type;	
	/// Initiator proterties, @see enum in_ble_initiator_prop.
    uint8_t prop;	
...
typedef struct 
{
	/// Periodic synchronization type, @see enum in_ble_per_sync_type.
    uint8_t type;	
...
typedef struct 
{
	...
	/// Authentication,@see enum ble_auth.
	uint8_t auth;	
...
typedef struct 
{
	/// Prefered PHY for transmission, @see enum in_ble_phy.
	uint8_t tx_phy;		
	/// Prefered PHY for reception, @see enum in_ble_phy.
	uint8_t rx_phy;		
	/// Prefered coding scheme for CODED PHY options, @see enum in_ble_coded_phy_opt.
	uint8_t phy_opt;	
} ble_conn_pref_phy_t;
...
typedef struct {
	/// Security Level, @see enum in_ble_att_svc_perm_mask
    uint8_t  sec_lvl;
...
typedef struct{
    ...
    /// GAP Specific Error of in_ble_error
	uint8_t status;
} ble_set_dev_info_cfm_t;
 ...
 *
 * @param[in] hdl				Ble stack handle; return from the stack initialization 
 * @param[in] callback		API completed callback function; NUL means blocking call.
 *
 ...
 *
 * @param[in] conidx				Connection index.
 * @param[in] reason				Reason of disconnection, @see enum in_ble_disc_reason.
 * 
 *
 * @return IN_BLE_ERR_NO_ERROR if successful, otherwise failed. @see enum in_ble_err_t 
 ****************************************************************************************
 ...
 *
 * @param[in] conidx				Connection index  
 * @param[in] p_cfm	       req, @see struct  in_ble_get_dev_info_cfm_t.
 *
 * @return IN_BLE_ERR_NO_ERROR if successful, otherwise failed. @see enum in_ble_err_t 
 ****************************************************************************************
 */
...
int in_ble_conn_set_dev_info_cfm(void *hdl, int conidx, ble_set_dev_info_cfm_t *p_cfm);
