...
static __inline void sdr_abort(uint8_t pulse_width) 
{
	uint32_t reg = IPMAC_REG_BLE_ABORT_REQ_CTL_BLE_ABORT_REQ|
								((pulse_width & IPMAC_REG_BLE_ABORT_REQ_CTL_BLE_ABORT_PULSE_DUR_MASK) << IPMAC_REG_BLE_ABORT_REQ_CTL_BLE_ABORT_PULSE_DUR_SHIFT) |
									IPMAC_REG_BLE_ABORT_REQ_CTL_BLE_ABORT_ERROR_FLAG_SET;
	WR_WORD(IPMAC_REG_BLE_ABORT_REQ, reg);		
}
...
{
	uint32_t reg = da | (mask << 	IPMAC_REG_SDR_DEST_ADDR_TO_MATCH_CTL_SDR_DEST_ADDR_MATCH_MASK_SHIFT);
	WR_WORD(IPMAC_REG_SDR_DEST_ADDR_TO_MATCH, reg);
}

static __inline void sdr_rx_sa_ctl(int idx, uint16_t sa, uint16_t mask) 
{
	uint32_t reg = sa | (mask << IPMAC_REG_SDR_SRC_ADDR_TO_MATCH_0_CTL_SDR_SRC_ADDR_0_MATCH_MASK_SHIFT);
...
static __inline void sdr_ignore_timestamp(int en)
{
	if (en)
		WR_WORD(IPMAC_REG_SDR_MISC_CTRL, (IPMAC_REG_SDR_MISC_CTRL_CTL_SDR_EN|IPMAC_REG_SDR_MISC_CTRL_CTL_SDR_IGNORE_TIMESTAMP|IPMAC_REG_SDR_MISC_CTRL_CTL_SDR_IGNORE_TIMESTAMP_FINE));
	else
		WR_WORD(IPMAC_REG_SDR_MISC_CTRL, 0);
}
 ...
 *
 * @param[in] mode									Transfer mode, @see enum sdr_mode
 * @param[in] slv_ack								Slave ack master's TX
 * @param[in] aa										Access address (common address shared by all the nodes)
 * @param[in] chn									Channel index (0 ~ 39),
 * @param[in] txr										Tx physical rate, @see enum sdr_phy_rate.
 * @param[in] rxr										Rx physical rate, @see enum sdr_phy_rate.
