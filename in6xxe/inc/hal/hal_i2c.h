...
typedef void (* i2c_evt_handler_t)(void *arg,  int err_code);
...
typedef enum i2c_stop_bit_e {
    I2C_STOP_BIT_NONE = 0,
    I2C_STOP_BIT_STOP,
}i2c_stop_bit_t;

typedef enum i2c_start_bit_e {
    I2C_START_BIT_NONE,
    I2C_START_BIT_START,
    I2C_START_BIT_RESTART,
}i2c_start_bit_t;

typedef enum i2c_role_e {
    I2C_ROLE_MASTER = 0,
    I2C_ROLE_SLAVE,
}i2c_role_t;
...
enum i2c_slave_event {
	I2C_SLAVE_READ_PROCESSED, // master read data from slave
    I2C_SLAVE_READ_DONE, // master read done
	I2C_SLAVE_WRITE_RECEIVED, // master write data to slave
	
};

typedef struct i2c_slave_client_s {
    void *i2c_hdl;
    void *priv;
}i2c_slave_client_t;

typedef int (*i2c_slave_cb_t)(struct i2c_slave_client_s *slave, enum i2c_slave_event event, uint8_t *val);

#define I2C_SLAVE_EVT_HANDLED   0
#define I2C_SLAVE_EVT_UNHANDLED 1
...
static __inline void i2c_slave_stop_det_enable(uint32_t i2c_base)
{
    uint32_t reg = RD_WORD(i2c_base + I2C_REG_CON_OFS);
...
static __inline void i2c_write(uint32_t i2c_base, uint8_t data, int read, int stop, int restart)
{
    uint16_t reg;
 ...
 * @param[in] cfg            The pointer to the I2c configuration.

 * @return Handle to the I2c driver, NULL means open failed. 
 ****************************************************************************************
 */
void *hal_i2c_open(int id, i2c_role_t role, i2c_speed_t speed,  bool dma_en);
 ...
 * @param[in] tmo            Timeout for block & poll call, 0: means wait forever
 ...
 ****************************************************************************************
 */
int hal_mi2c_write_blocking(void *hdl, uint8_t sar, uint8_t *buffer, uint16_t buffer_len, uint32_t tmo);    
int hal_mi2c_write_poll(void *hdl, uint8_t sar, uint8_t *buffer, uint16_t buffer_len, uint32_t tmo);    
int hal_mi2c_write_async(void *hdl, uint8_t sar, uint8_t *buffer, uint16_t buffer_len, void *arg, void (*callback)(void *arg, int error));    
 ...
 ****************************************************************************************
 */
int hal_mi2c_read_blocking(void *hdl, uint8_t sar, uint8_t *buffer, uint16_t buffer_len, uint32_t tmo);
int hal_mi2c_read_poll(void *hdl, uint8_t sar, uint8_t *buffer, uint16_t buffer_len, uint32_t tmo);
int hal_mi2c_read_async(void *hdl, uint8_t sar, uint8_t *buffer, uint16_t buffer_len, void *arg, void (*callback)(void *arg, int error));
 ...
 ****************************************************************************************
 */
int hal_mi2c_write_read_blocking(void *hdl, uint8_t tar, uint8_t *wr_buf, uint16_t wr_len, uint8_t *rd_buf, uint16_t rd_len, uint32_t tmo);
int hal_mi2c_write_read_poll(void *hdl, uint8_t tar, uint8_t *wr_buf, uint16_t wr_len, uint8_t *rd_buf, uint16_t rd_len, uint32_t tmo);
int hal_mi2c_write_read_async(void *hdl, uint8_t tar, uint8_t *wr_buf, uint16_t wr_len, uint8_t *rd_buf, uint16_t rd_len, void *arg, void (*callback)(void *arg, int error));


/**
 ****************************************************************************************
 * @brief Function for checking the i2c device state
 *
 * @param[in] hdl            The handle from the previous "open" function.   
 *
 ...
 ****************************************************************************************
 */
bool hal_i2c_is_busy(void *hdl);


/**
 ****************************************************************************************
 * @brief Function for changing the i2c device interrupt priority
 *
 * @param[in] hdl            The handle from the previous "open" function.   
 *
 * @return This function has no return. 
 ****************************************************************************************
 */
void hal_i2c_set_irq_priority(void *hdl, int pri);


/**
 ****************************************************************************************
 * @brief I2c slave register.
 * @note This function set i2c controler as slave .
 * @param[in] hdl            The handle from the previous "open" function.   
 * @param[in] address        The slave address.
 * @param[in] slave_cb       Write & read event callback.
 * @param[in] slave          Argument that user wants to pass to the call back function
 *
 * @return @see enum i2c_error for the possible return code. 
 ****************************************************************************************
  */
int hal_si2c_register(void *hdl, uint8_t address, i2c_slave_cb_t slave_cb, struct i2c_slave_client_s *slave);
